...
/**
     ...
     * @param  string $query String to be included in donator&#39;s first name or last name (optional)
    ...
    public function postFinanceReportsWithHttpInfo($report_type, $post_finance_reports_request = null, string $contentType = self::contentTypes['postFinanceReports'][0])
    {
        ...
        try {
            ...
            switch($statusCode) {
                case 200:
                    if ('\StevenBuehner\ChurchTools\Model\PostFinanceTurnoverReport200Response' === '\SplFileObject') {
                        $content = $response->getBody(); //stream goes to serializer
                    } else {
                        $content = (string) $response->getBody();
                        if ('\StevenBuehner\ChurchTools\Model\PostFinanceTurnoverReport200Response' !== 'string') {
                            $content = json_decode($content);
                        }
                    }

                    return [
                        ObjectSerializer::deserialize($content, '\StevenBuehner\ChurchTools\Model\PostFinanceTurnoverReport200Response', []),
                        $response->getStatusCode(),
                        $response->getHeaders()
                    ];
            }
     ...
     *
     * @param  \StevenBuehner\ChurchTools\Model\PostFinanceTurnoverReportRequest $post_finance_turnover_report_request  (optional)
